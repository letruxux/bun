			...
			<ContextMenu>
								...
								<span className="mt-0.5 flex truncate font-normal text-sm">
									...
									<span className="flex items-center">
										<img
											...
											src={new ImageWeserv(
												urlToImg(item.url, settings.explicitAvatars),
											)
												.setWidth(40)
												.setHeight(40)
												.toString()}
				...
				<ContextMenuContent>
					<ContextMenuItem onClick={() => addHiddenPostId(item.superUniqueId)}>
						<HiOutlineEyeOff className="h-4 w-4 text-foreground" /> hide this
						post
					</ContextMenuItem>
					<ContextMenuItem variant="destructive">
						<HiOutlineEyeOff className="h-4 w-4 text-foreground" /> hide all
						posts from this domain
					</ContextMenuItem>
				</ContextMenuContent>
			</ContextMenu>
		</a>
						...
						<DropdownMenuContent>
							<DropdownMenuGroup>
								<DropdownMenuItem
									className="flex"
									onClick={() =>
										updateRSSSettings({ showImages: !settings.showImages })
									}
								>
									<Checkbox checked={settings.showImages}></Checkbox> show
									images
								</DropdownMenuItem>
								<DropdownMenuItem
									className="flex"
								...
								>
									<Checkbox checked={settings.hidePostsWithNoDate}></Checkbox>{" "}
									hide posts with no date
								</DropdownMenuItem>
								<DropdownMenuItem
									className="flex"
									onClick={() =>
