  ...
  return (
    ...
    >
      <ContextMenu>
        ...
        <ContextMenuContent>
          {readArticleIds.includes(item.superUniqueId) && (
            <ContextMenuItem onClick={() => setArticleUnread(item.superUniqueId)}>
              <HiOutlineEye className="h-4 w-4 text-foreground" /> mark as unread
            </ContextMenuItem>
          )}
          ...
          <Button
            ...
            onClick={() => {
              ...
              for (const feed of feeds) {
                if (!readArticleIds.includes(feed.superUniqueId)) {
                  setArticleRead(feed.superUniqueId);
                }
              }
            }}
            disabled={
              !(feeds.find((e) => !readArticleIds.includes(e.superUniqueId)) ?? null)
            }
          >
            <CircleIcon className="h-4 w-4 fill-white" />
          </Button>
        </div>
        {loading.size > 0 && (
